...
namespace SiamCross.Models.Tools
{
    public class XmlCreator
    {
        public string CreateDdim2Xml(Ddim2Measurement dbMeasurementItem)
        {
            ...
            //builder.Append(String.valueOf(placementType));                                                // wSensor == ????? => placementType == ?????
            builder.Append("\" />");
            builder.Append("\r\n          ");
        ...
        private double[] GetMovement(double[] discrets)
        {
            var movement = new List<double>();
            for (int i = 0; i <= discrets.Length; i += 2)
            {
                movement.Add(discrets[i]);
            }
